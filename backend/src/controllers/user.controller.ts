...
export const getUserProfile = async (req:Request, res: Response) => {
 try{
  ...
  const profile = await client.user.findUnique({
   where: {id: userId},
   select: {firstName: true, lastName: true, userName: true, emailAddress: true, profileImage: true}
  })
...
export const updateUserProfile = async (req: Request, res: Response) => {
 try{
  ...
  return res.status(200).json({message: "Profile updated successfully.", updatedUserProfile})
 } catch (e){
...
try{
 ...
 if (!isMatch) {
  return res.status(400).json({message: "Current passwords don't match."})
 }
