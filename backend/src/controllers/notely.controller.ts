...
export const getAllDeletedNotes = async (req:Request, res:Response) => {
 try {
  ...
  return res.status(200).json({message: "Deleted notes fetched successfully", data: deletedNotes,})
...
export const getSpecificNote = async (req: Request, res: Response) => {
 try{
  ...
  return res.status(200).json({message: "Note displayed successfully", data: note,})
 } catch (e) {
...
export const restoreDeletedNote = async (req: Request, res: Response) => {
  try{
    ...
    const restoredNote = await client.note.update({where: {id: noteId}, data: {isDeleted: false}})
    res.status(200).json({message: "Note restored successfully.", restoredNote})
  } catch (e) {
    console.log("Error restoring blog")
