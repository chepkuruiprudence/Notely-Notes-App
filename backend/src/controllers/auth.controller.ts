...
export const logInUser = async (req:Request, res: Response) => {
 try{
  ...
  const user = await client.user.findFirst({
   where: {
    OR: [
     {userName: identifier}, {emailAddress: identifier}]
  ...
  const { passWord:userPassword, datejoined, updatedAt, ...userDetails } = user;
  ...
  return res.status(201).json({message: "Login Successful", token, userDetails})
 } catch (e) {
